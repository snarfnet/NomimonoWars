...
# Find the correct screenshot set for this display type
print(f'Looking for display type: {args.display_type}')
r = api('GET', f'/appStoreVersionLocalizations/{loc_id}/appScreenshotSets')
set_id = None
for s in r.json().get('data', []):
    if s['attributes']['screenshotDisplayType'] == args.display_type:
        set_id = s['id']
        print(f'Found matching set: {set_id}')
...
if not set_id:
    print(f'No existing set found, creating new one for {args.display_type}')
