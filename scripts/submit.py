...
review_notes = """This build addresses the iPad Air 11-inch layout issue reported under Guideline 4. The entire app content is now placed inside explicit top and bottom safe-area padding instead of relying on an edge-to-edge custom layout. The tab bar and adaptive AdMob banner sit above the home indicator area, and the header sits below the status area. iPad now requires full screen and is limited to portrait and upside-down portrait to avoid the crowded landscape layout. The bottom navigation still directly shows the three tabs named in the metadata: 新商品, ランキング, and ニュース. Saved articles are opened from the bookmark button in the header.
